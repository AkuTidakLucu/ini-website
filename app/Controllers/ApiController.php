...
{
    private $pertanyaan_umum_prioritas;
    ...
    public function __construct()
    {
        ...
        // $json_pertanyaan = file_get_contents(filename: APPPATH . 'Knowladge/data.json');
        // $this->pertanyaan_umum_prioritas = json_decode($json_pertanyaan, true)['pertanyaan_umum_prioritas'];

        // $modelData = new Pengetahuan();
        // $this->pertanyaan_umum_prioritas = json_decode($modelData->findAll()["pengetahuan"], true);

    session();

    $modelData = new Pengetahuan();
    $semuaData = $modelData->findAll();

    // Simpan sebagai ARRAY (jangan di json_encode di sini)
    $this->pertanyaan_umum_prioritas = array_column($semuaData, 'pengetahuan') ?: [];
...
// OOT;
        ...
        try {
            if ($lastChat !== null) {
                if (count($data) > 3) {
                    ...
                    $response = $client->generativeModel(modelName: ModelName::GEMINI_2_5_FLASH)
        ...
        return redirect()->to('/chat/c/' . $chatId)->with('speak', 'ongoing');
    }
    ...
    public function newChat()
    {
        ...
        return redirect()->to('/chat/c/' . $chatId . '?ai=onMessage');
    }
